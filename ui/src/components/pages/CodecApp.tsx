...
const NAV_LINK_CLASSES =
  "group flex min-h-11 items-center gap-3 border-l-2 border-transparent px-4 text-sm font-medium text-muted transition duration-200 hover:border-line hover:bg-field hover:text-ink active:translate-y-px focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-primary";
const ACTIVE_NAV_LINK_CLASSES = "border-accent bg-field font-semibold text-ink";
    ...
    <div data-testid="codec-app" className="min-h-dvh md:flex">
      ...
      >
        ...
        <footer data-testid="sidebar-footer" className="border-line mt-auto flex justify-end border-t p-4">
          <ThemeControl dark={dark} onToggle={() => setDark((value) => !value)} />
        </footer>
      </aside>
      ...
      {drawerOpen && (
        <div data-testid="mobile-drawer-layer" className="fixed inset-0 z-30 md:hidden">
          ...
          <aside
            ...
            className="border-line bg-panel relative flex h-dvh w-4/5 max-w-80 flex-col border-r shadow-2xl"
            onKeyDown={trapFocus}
          >
      ...
      <main id="main-content" data-testid="app-main" className="min-w-0 flex-1 px-4 pt-8 pb-12 md:px-8 md:pt-12">
        <header data-testid="tool-header" className="border-line mb-7 border-b pb-6">
          <h1 className="font-display text-ink text-3xl leading-tight font-bold tracking-tight text-balance md:text-5xl">
            {tool.title}
          </h1>
